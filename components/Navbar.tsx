...
import { Phone, MessageCircle, UserPlus, Video, Copy } from "lucide-react";
...
export default function Navbar({ appearance }: NavbarProps) {
  ...
  const usesPastHero =
    resolvedAppearance === "home-scroll" ||
    resolvedAppearance === "page-gradient-scroll";
        ...
        <div className="grid grid-cols-3 lg:grid-cols-3 items-center h-full transition-all duration-500 w-full">
          ...
          <nav className="hidden lg:flex items-center gap-20 lg:ms-30 justify-center">
            {NAV_LINKS.filter((item) =>
              ["ABOUT", "PLACES", "CURATIONS"].includes(
          ...
          <div className="flex items-center  justify-start lg:justify-center whitespace-nowrap">
            <Link
              href="/"
              style={{ fontFamily: "TheSeasons, serif" }}
              className={`lg:text-5xl md:text-5xl sm:text-4xl text-4xl font-semibold tracking-[4px] ${
          ...
          <div className="hidden lg:flex items-center justify-center gap-4 w-full">
            ...
            <Link href="/enquiry" className="shrink-0">
              <button
                style={{ fontFamily: "TheSeasons, serif" }}
                className="bg-black lg:me-30  px-4 py-2.5 text-[13px] rounded-[3px] font-semibold text-white hover:bg-gray-400 hover:text-black cursor-pointer transition whitespace-nowrap"
              >
                LET’S CONNECT
              </button>
            </Link>
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            >
              {/* Mobile Drawer Header (ALWAYS visible) */}
              ...
              <nav className="px-6 py-4 space-y-4 flex-1 bg-white overflow-y-auto">
                <AnimatePresence mode="wait">
                  ...
                  {/* Destinations Level 1 - countriees */}
                    ...
                    >
                      ...
                      {BY_DESTINATION.map((dest) => (
                        <button
                          key={dest.slug}
                          onClick={() => {
  router.push(`/experience-types/destination/${dest.slug}`);
  setOpen(false);
}}
                      ...
                      >
                        ...
                        {EXPERIENCES_DATA.categories.map((cat) => (
                          <button
                            key={cat.id}
                            onClick={() =>
                              setSelectedExperienceCategory(cat.id)
                            }
                            className="block w-full text-left text-md tracking-[1.5px] font-brandon font-semibold text-gray-600  py-2 uppercase"
                          >
                            {cat.label}
                  ...
                  {mobileMenuLevel === "experiences" &&
                    selectedExperienceCategory && (
                      ...
                      >
                        ...
                        <h2 className="text-2xl font-medium tracking-[2.5px] mb-6 text-black uppercase ">
                          {
                            EXPERIENCES_DATA.categories.find(
                              (c) => c.id === selectedExperienceCategory,
                            )?.label
                          }
                        </h2>
